module Xmobar.Plugins.Monitors.MyAlsa
  ( MyAlsa (..),
  )
where
...
import qualified Xmobar.Plugins.Monitors.MyVolume as Volume

data MyAlsa = MyAlsa String String Args
  deriving (Show, Read)

instance Exec MyAlsa where
  alias (MyAlsa m c _) = "myalsa:" ++ m ++ ":" ++ c
  start (MyAlsa m c a) = startAlsaPlugin m c a
...
alsaReaderThread mixerName alsaCtlPath outputCallback mvar =
  let createProc =
        (proc "stdbuf" ["-oL", alsaCtlPath, "monitor", mixerName])
          { std_out = CreatePipe
          }
...
createRateLimiter interval = do
  ...
  let limiter = do
        ...
        case prevTime0 of
          Just prevTime
            | diff <- interval - (curTime `diffUTCTime` prevTime),
              diff > 0 ->
                do
                  threadDelayNDT diff
