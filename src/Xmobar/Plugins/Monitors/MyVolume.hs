module Xmobar.Plugins.Monitors.MyVolume
  ( runVolume,
    runVolumeWith,
    volumeConfig,
    options,
...
data VolumeOpts = VolumeOpts
  { onString :: String,
    ...
    volumeCharPattern :: Maybe String,
    offChar :: String,
...
-- volume
getVolStatus ::
  -- | Low volume threshold, in [0,100]
  Float ->
  -- | High volume threshold, in  [0,100]
  Float ->
...
options =
  [ Option "O" ["on"] (ReqArg (\x o -> o {onString = x}) "") "",
    ...
    Option
      ...
      ( ReqArg
          ( \x o ->
              o {volumeCharPattern = Just x}
          )
          ""
      )
      "",
    Option "x" ["offChar"] (ReqArg (\x o -> o {offChar = x}) "") "",
...
showCharPattern :: Maybe String -> Float -> Monitor String
showCharPattern Nothing _ = return ""
showCharPattern (Just str) x = return [str !! convert (100 * x)]
  where
    convert val
      | t <= 0 = 0
      ...
      | otherwise = t
      where
        upperThreshold = length str - 1
        t = round val `div` (100 `div` length str)

formatVolDCStr :: Maybe String -> Integer -> Integer -> Integer -> Monitor String
formatVolDCStr cpat lo hi v =
  showCharPattern cpat $ percent v lo hi
...
switchHelper ::
  ...
  VolumeStatus ->
  Monitor String
switchHelper opts cHelp strHelp vs =
  return $
...
formatDb opts dbi = do
  ...
  let db = fromIntegral dbi / 100.0
      ...
      startColor
        | db >= highDbThresh opts = colorHelper h
...
runVolumeWith opts mixerName controlName = do
  ...
  cpat <-
    if isVolOff sw
      then return $ offChar opts
      else liftMonitor $ liftM3 (formatVolDCStr $ volumeCharPattern opts) lo hi val
  ...
  parseTemplate [p, b, v, d, s, ipat, vs, cpat]
  where
    readMixer =
      AE.catch
        ( withMixer mixerName $ \mixer -> do
